...
pip3 install wheel gunicorn flask flask_cors aws-xray-sdk boto3 aws-opentelemetry-distro flask-sqlalchemy mysql-connector-python pymysql cryptography requests
...
opentelemetry-instrument gunicorn --capture-output --log-level debug --error-logfile gunicorn_error.log --bind 0.0.0.0:5002 wsgi:app --daemon
