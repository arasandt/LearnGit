...
def get_users():
    ...
    try:
        ...
        fact = f"Could not fetch cat fact for you..."

    users = User.query.all()
    user_data = "\n".join(
        [f"{count +1 }: {user.username}" for count, user in enumerate(users)]
    )
    template = env.get_template("users.html")
    return template.render(user_data=user_data, fact=fact, **common_vars)
